-- Drop the old constraint on users table current_level
ALTER TABLE public.users DROP CONSTRAINT IF EXISTS users_current_level_check;

-- Add new constraint that accepts CEFR levels
ALTER TABLE public.users ADD CONSTRAINT users_current_level_check 
CHECK (current_level IN ('A1', 'A2', 'B1', 'B2', 'C1', 'C2', 'beginner', 'elementary', 'intermediate', 'upper_intermediate', 'advanced', 'proficient'));

-- Update existing user levels to match CEFR format
UPDATE public.users 
...
-- Now apply the strict CEFR constraint
ALTER TABLE public.users DROP CONSTRAINT users_current_level_check;
ALTER TABLE public.users ADD CONSTRAINT users_current_level_check 
...
-- Create level_progression table
...
ON public.generated_content FOR UPDATE USING (auth.uid() = user_id);

-- RLS Policies for level_assessments
CREATE POLICY "Users can view their own level assessments" 
ON public.level_assessments FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own level assessments" 
ON public.level_assessments FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own level assessments" 
ON public.level_assessments FOR UPDATE USING (auth.uid() = user_id);

-- RLS Policies for level_progression
CREATE POLICY "Users can view their own level progression" 
ON public.level_progression FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create their own level progression" 
ON public.level_progression FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own level progression" 
ON public.level_progression FOR UPDATE USING (auth.uid() = user_id);
...
SELECT id, current_level FROM public.users
